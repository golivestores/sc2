...
For every `effects/NNN-name/` folder, produces a drop-in
`effects/NNN-name/NNN-name.zip`. The source viewer reads the effect's real
files directly over HTTP and has no generated source dependency.
...
# Stale generated viewer artifacts are never included in ZIP downloads.
...
def package_one(effect_dir: Path):
    files = collect_files(effect_dir)
    # Zip is the user-facing download — keep ONLY what the block needs to run.
    #    Exclude sc2-internal artifacts:
    #      - stale generated viewer artifacts
    #      - meta.json (gallery card metadata)
    #      - the zip itself + any other sibling .zip
    files_for_zip = [
        p for p in files
        if p.name not in GENERATED_NAMES and not is_sc2_metadata(p, effect_dir)
    ]
    zip_path, raw_bytes = write_zip(effect_dir, files_for_zip)
    return zip_path, len(files_for_zip), raw_bytes


# === Tag taxonomy (locked — see TAGS.md) ====================================
# The 5-axis spec lives in tag-axis.json at repo root. We load it once at
# import time and derive both the validator's per-axis tag sets and the
# cardinality rules from it. effects/tag-axis.js (used by gallery + viewer)
# is generated from the SAME json by rebuild-index.py, so all three consumers
# (Python validator + two HTML pages) stay in sync from one edit point.
_TAG_AXIS_JSON = ROOT / "tag-axis.json"
try:
    _spec = json.loads(_TAG_AXIS_JSON.read_text(encoding="utf-8"))
except FileNotFoundError:
    print(f"ERROR: missing {_TAG_AXIS_JSON}. Restore from git.", file=sys.stderr)
    sys.exit(1)
except json.JSONDecodeError as _e:
    print(f"ERROR: tag-axis.json is malformed: {_e}", file=sys.stderr)
    sys.exit(1)

# Per-axis sets keyed by axis name (e.g. "bankuai"). Used by _validate_tags.
TAG_AXIS_SETS: dict[str, set[str]] = {a["name"]: set(a["tags"]) for a in _spec["axes"]}
# Cardinality + label info, parallel order to spec for stable error messages.
_AXIS_RULES = [
    {"name": a["name"], "label": a["label"],
     "required": int(a["required"]), "max": int(a["max"])}
    for a in _spec["axes"]
]
ALL_KNOWN_TAGS = set().union(*TAG_AXIS_SETS.values())
...
def _validate_tags(tags: list[str]) -> list[str]:
    ...
    # Bucket into axes + check cardinality.
    for i, rule in enumerate(_AXIS_RULES, start=1):
        bucket = [t for t in tags if t in TAG_AXIS_SETS[rule["name"]]]
        n = len(bucket)
        if rule["required"] == 1 and n != 1:
            errors.append(f"axis {i} {rule['label']}: need exactly 1, got {n} ({bucket})")
        elif n > rule["max"]:
            errors.append(f"axis {i} {rule['label']}: max {rule['max']} allowed, got {n} ({bucket})")
...
def main():
    ...
    # are regenerated by rebuild-index.py, which only warns and continues), so
    ...
    print(f"packaging {len(targets)} effect(s) -> .zip\n")
    total_files = 0
    total_bytes = 0
    total_zip_bytes = 0
    for d in targets:
        zip_path, n, raw = package_one(d)
        zip_bytes = zip_path.stat().st_size
        total_files += n
        total_bytes += raw
        total_zip_bytes += zip_bytes
        print(f"  ok  {d.name:<40s}  {n:>3} files  {raw/1024:>7.1f} KB -> "
              f"{zip_bytes/1024:>7.1f} KB zip")

    print(f"\ntotal: {total_files} files, {total_bytes/1024:.1f} KB raw -> "
          f"{total_zip_bytes/1024:.1f} KB zipped")
